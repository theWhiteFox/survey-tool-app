...
export default function ResponsesTable({ questionResponse }: ResponsesTableProps) {
    ...
    const chartData = Object.entries(questionResponse.percentages).map(([option, percentage]) => ({
        name: option || "(No Answer)",
        value: percentage,
        totalVotes: questionResponse.option_totals[option]
    }))
                ...
                <div className="inline-block align-middle">
                    <div className="rounded-lg bg-gray-50 p-2 outline-1 md:pt-0">
                        <h3 className="text-xl font-semibold mb-4 text-gray-900 pt-4">
                            Results for: Question ID {questionResponse.question_id}
                        </h3>
                                ...
                                <div className="flex items-center justify-between border-b pb-4">
                                    <div>
                                        <div className="mb-2 flex items-center text-gray-500">
                                            🔗 Question ID: {questionResponse.question_id}
                                        </div>
                                        <p className="text-sm text-gray-500">
                                            📝 Total Votes: {questionResponse.total_votes}
                                        </p>
                                        <p className="text-sm text-gray-500">
                                            ...
                                            {Object.entries(questionResponse.percentages).map(([option, percentage]) => (
                                                <span key={option} className="block ml-4">
                                                    {option || "(No Answer)"}: {percentage}% ({questionResponse.option_totals[option] ?? 0} votes)
                                                </span>
                                            ))}
                                        </p>
                                    </div>
